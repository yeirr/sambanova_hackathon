...
def init_semantic_cache() -> SemanticCache:
    ...
    # Sanity check with test query.
    # test_query = "tell me about human gut microbiome"
    # semantic_cache.check(prompt=test_query, num_results=1)[0]
...
async def main(timeout: int = 30) -> None:
    ...
    with bottom():
        st.write(
            "Language models can hallucinate, please verify output with trusted sources."
        )
